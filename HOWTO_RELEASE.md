...
mvn -Dmaven.repo.local=/tmp/repo \
    -DdevelopmentVersion=2.6.3-SNAPSHOT \
    -DreleaseVersion=2.6.2 \
    -Dtag=v2.6.2 \
...
# copy changed site
grep -lr 2.6.3-SNAPSHOT | xargs sed -i 's/2.6.3-SNAPSHOT/2.6.2/g'
git add .
...
6. `mvn -Dmaven.repo.local=/tmp/repo -DdevelopmentVersion=2.6.3-SNAPSHOT -DreleaseVersion=2.6.2 -Dtag=v2.6.2 -Pdist release:prepare`
...
grep -lr 2.6.3-SNAPSHOT | xargs sed -i 's/2.6.3-SNAPSHOT/2.6.2/g'
git add .
git commit -m '[site] Jolokia site for 2.6.2'
git push origin HEAD
```
