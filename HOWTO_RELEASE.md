...
mvn -Dmaven.repo.local=/tmp/repo \
    -DdevelopmentVersion=2.2.7-SNAPSHOT \
    -DreleaseVersion=2.2.6 \
    -Dtag=v2.2.6 \
...
# copy changed site
grep -lr 2.2.7-SNAPSHOT | xargs sed -i 's/2.2.7-SNAPSHOT/2.2.6/g'
git add .
...
6. `mvn -Dmaven.repo.local=/tmp/repo -DdevelopmentVersion=2.2.7-SNAPSHOT -DreleaseVersion=2.2.6 -Dtag=v2.2.6 -Pdist release:prepare`
