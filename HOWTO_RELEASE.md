...
mvn -Dmaven.repo.local=/tmp/repo \
    -DdevelopmentVersion=2.6.4-SNAPSHOT \
    -DreleaseVersion=2.6.3 \
    -Dtag=v2.6.3 \
...
# copy changed site
grep -lr 2.6.4-SNAPSHOT | xargs sed -i 's/2.6.4-SNAPSHOT/2.6.3/g'
git add .
...
6. `mvn -Dmaven.repo.local=/tmp/repo -DdevelopmentVersion=2.6.4-SNAPSHOT -DreleaseVersion=2.6.3 -Dtag=v2.6.3 -Pdist release:prepare`
...
grep -lr 2.6.4-SNAPSHOT | xargs sed -i 's/2.6.4-SNAPSHOT/2.6.3/g'
git add .
git commit -m '[site] Jolokia site for 2.6.3'
git push origin HEAD
```
