...
mvn -Dmaven.repo.local=/tmp/repo \
    -DdevelopmentVersion=2.6.2-SNAPSHOT \
    -DreleaseVersion=2.6.1 \
    -Dtag=v2.6.1 \
...
# copy changed site
grep -lr 2.6.2-SNAPSHOT | xargs sed -i 's/2.6.2-SNAPSHOT/2.6.1/g'
git add .
...
6. `mvn -Dmaven.repo.local=/tmp/repo -DdevelopmentVersion=2.6.2-SNAPSHOT -DreleaseVersion=2.6.1 -Dtag=v2.6.1 -Pdist release:prepare`
