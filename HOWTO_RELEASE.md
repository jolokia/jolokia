...
mvn -Dmaven.repo.local=/tmp/repo \
    -DdevelopmentVersion=2.4.1-SNAPSHOT \
    -DreleaseVersion=2.4.0 \
    -Dtag=v2.4.0 \
...
# copy changed site
grep -lr 2.4.1-SNAPSHOT | xargs sed -i 's/2.4.1-SNAPSHOT/2.4.0/g'
git add .
...
6. `mvn -Dmaven.repo.local=/tmp/repo -DdevelopmentVersion=2.4.1-SNAPSHOT -DreleaseVersion=2.4.0 -Dtag=v2.4.0 -Pdist release:prepare`
