...
import org.jolokia.client.EntityUtil;
...
public class JdkHttpClient implements HttpClientSpi<HttpClient> {
    ...
    J4pClientBuilder.Configuration config;
    private final URI jolokiaAgentUrl;

    public JdkHttpClient(HttpClient client, J4pClientBuilder.Configuration configuration) {
        this.client = client;
    ...
    JSONStructure execute(REQ pRequest, HttpMethod method, Map<J4pQueryParameter, String> parameters, JolokiaTargetConfig targetConfig)
    ...
    @Override
    public <REQ extends JolokiaRequest, RES extends JolokiaResponse<REQ>>
    JSONStructure execute(List<REQ> pRequests, Map<J4pQueryParameter, String> parameters, JolokiaTargetConfig targetConfig)
            throws IOException, J4pException {
    ...
    HttpRequest prepareRequest(REQ pRequest, HttpMethod method, Map<J4pQueryParameter, String> parameters, JolokiaTargetConfig pTargetConfig) {
        JolokiaTargetConfig targetConfig = determineTargetConfig(pRequest, pTargetConfig);
        HttpMethod selectedMethod = determineHttpMethod(pRequest, method, targetConfig);
        String queryParams = EntityUtil.toQueryString(parameters);
        ...
        if (selectedMethod.equals(HttpMethod.GET)) {
            ...
            if (parts != null) {
                ...
                URI uri = prepareFullUrl(requestPath.toString(), queryParams);
        ...
        if (builder == null) {
            ...
            JSONObject requestContent = getJsonRequestContent(pRequest, targetConfig);
            HttpRequest.BodyPublisher publisher = HttpRequest.BodyPublishers.ofString(requestContent.toJSONString(), charset);

            builder = HttpRequest.newBuilder().uri(prepareFullUrl(jolokiaAgentUrl.getPath(), queryParams)).POST(publisher);
            // for POST only
     ...
     */
    private <REQ extends JolokiaRequest>
    HttpRequest prepareRequests(List<REQ> pRequests, Map<J4pQueryParameter, String> parameters, JolokiaTargetConfig pTargetConfig) {
        String queryParams = EntityUtil.toQueryString(parameters);
        Charset charset = config.contentCharset() == null ? StandardCharsets.UTF_8 : config.contentCharset();
        ...
        for (REQ request : pRequests) {
            ...
            JSONObject requestContent = getJsonRequestContent(request, determineTargetConfig(request, pTargetConfig));
        ...
        HttpRequest.Builder builder = HttpRequest.newBuilder().uri(prepareFullUrl(jolokiaAgentUrl.getPath(), queryParams)).POST(publisher);
    ...
    private JSONStructure execute(HttpRequest httpRequest, JolokiaRequest jolokiaRequest, String requestType) throws IOException, J4pException {
        ...
        try {
            ...
            try (InputStream body = response.body()) {
                ...
                return EntityUtil.parseJsonResponse(body, Charset.forName(encoding.get()));
     ...
     * @param <REQ>
     */
    private <REQ extends JolokiaRequest> JolokiaTargetConfig determineTargetConfig(REQ pRequest, JolokiaTargetConfig pTargetConfig) {
     ...
     * @param <REQ>
     */
    private <REQ extends JolokiaRequest> HttpMethod determineHttpMethod(REQ pRequest, HttpMethod method, JolokiaTargetConfig targetConfig) {
        if (method == null) {
     ...
     */
    private URI prepareFullUrl(String path, String queryParams) {
        try {
     ...
     * @return
     */
    private JSONObject getJsonRequestContent(JolokiaRequest pRequest, JolokiaTargetConfig pTargetConfig) {
