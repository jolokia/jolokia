...
`META-INF/jolokia/services` is a bit similar approach to Java standard `java.util.ServiceLoader` API, where services are _declared_ in `META-INF/services/interface-name`.
...
4. However, both `ObjectName` and ``ObjectInstance``'s class can be use to obtain a _key_ (or a _hint_) which determines that some MBeans may _share_ its `javax.management.MBeanInfo` with other MBeans.
