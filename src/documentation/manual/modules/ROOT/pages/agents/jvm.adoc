...
`single`:: A single thread only. This is the default value, but (as the name suggests) it limits the throughput of Jolokia Agent. This value should not be used in production configuration.
|`single`
...
(https only). Can be autodetected, when the `serverKey` is in PEM format.
|`RSA`
...
[options] are used for providing runtime information for attaching the agent:
    ...
    --canonicalNaming <t|f>         whether to use canonicalName for ObjectNames in 'list' or 'search' (default: false)
