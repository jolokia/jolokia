...
|Path to an allow list of patterns which are matched against possible
JMX service URL for incoming requests. If no white list is specified, no target URL is allowed at all
which effectively disables the proxy.
|`/opt/jolokia/jsr160-proxy-allowed-patterns.txt`, `classpath:/jsr160-proxy-allowed-patterns.txt`
...
here a Java class implementing `org.jolokia.core.api.LogHandler`
...
|`logHandlerName`
|A parameter which may be used to configure _logger name_ which may be used with some `org.jolokia.core.api.LogHandler` implementations.
...
`list` or `search` are
returned. By default this is the order of properties as specified
at registration time. If this option
is set to `true`, the so called _canonical order_
will be used, where the keys are sorted alphabetically. This
...
|`trustProxyHeaders`
|Normally, Jolokia can obtain the IP of the caller from Servlet or JDK HTTP Server API.
But when Jolokia Agent runs behind a reverse proxy, using such API gives us the IP address
of the proxy itself. Since Jolokia 2.6.1 headers like `Forwarded`, `X-Forwarded-For` and `X-Real-IP` are
read (in that order) and all are validated against configured "allowed client addresses" (in the
restrictor configuration). Normally, all found addresses are checked and all must be configured as trusted. +
With this option set to `true`, Jolokia trusts that the _first_ IP address in `Forwarded`/`X-Forwarded-For` header
is assumed to be set by a trusted reverse proxy. This means that all the following addresses are actually
addresses of downstream reverse proxies which are trusted.
|Default: `false`

|`useFetchMetadata`
|This option (enabled by default) tells Jolokia to process https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Fetch_metadata[Fetch Metadata] headers. These headers are sent by modern browsers in https://developer.mozilla.org/en-US/docs/Web/Security/Defenses/Secure_Contexts[secure contexts] (i.e., over https or for local addresses, which means these are _not_sent for addresses like `http://192.168.0.1` or `http://example.com`). These headers are browser hints for the type of navigation (top-level? `<img>`? `fetch()`/`xhr`?) and can be used to detect unwanted cross-site requests.
|Default: `true`
|===
...
the form of a JSON object, where the keys are product names and
