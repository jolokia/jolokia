...
`listKeys`:: A flag (defaults to `false`) to enable _optimized list response_. +
...
We can use a tool like https://nmap.org/ncat/[netcat,role=externalLink,window=_blank] to check the discovery:

[shell,subs="attributes,verbatim"]
----
$ nc -u localhost 24884
{"type":"query"}
{"agent_version":"{jolokia-version}","agent_id":"192.168.0.221-67980-7e985ce9-servlet",
...
enables _optimized list response_.
