...
Not much to add. The `JMXServiceURL` matches the same _recipe_ used at the server side to create and export the server side of the remote JMX Connector.
...
Jolokia doesn't implement _entire_ JMX Connector, only the client-side.

* `org.jolokia.client.jmxadapter.JolokiaJmxConnector` for `service:jmx:jolokia:` JMX Service URLs
* `org.jolokia.kubernetes.client.KubernetesJmxConnector` for `service:jmx:kubernetes:` JMX Service URLs

See the details in xref:clients.adoc#_jolokia_jmx_connectors[Jolokia MBeanServerConnection adapter].
