...
In Jolokia 2, `org.jolokia.service.jsr160.Jsr160RequestHandler` is detected as Jolokia service, so it's a matter of having `jolokia-service-jsr160.jar` on the CLASSPATH. This enables proxy support, but no target URL is permitted by default.
An allow list (by default it's empty) is used to select which target URLs are allowed to be used with JMX Connector Client
created by Jolokia.

`jolokia-agent-war.war` includes `WEB-INF/lib/jolokia-service-jsr160.jar`, so proxy mode is enabled. If you don't want to include proxy support, you have to repackage `jolokia-agent-war.war` without this library. But still, without an allow list configured, no target
Jolokia can be reached via this proxy.

You can configured a white list with patterns for all allowed JMX service URL in a Jolokia Request.
...
be an absolute path or a relative path if you know where your Jakarta EE server sets the current directory. `classpath:/resource.txt`
syntax is also supported, so the configuration file can be provided on the CLASSPATH.
* Set the system property `jolokia.jsr160ProxyAllowedTargets` to the path of the allow list.
* Set the environment variable `JOLOKIA_JSR160_PROXY_ALLOWED_TARGETS` to the path of the
allow list.
* Set the `jsr160ProxyAllowedTargets` option for JVM Agent.

By default no target URLs are allowed, but you can specify the allowed list like this:

----
# Allow access to remote JMX Connector Servers using RMI protocol using stubs obtained from RMI registries
# running on localhost or hosts from .dev domain on port 1099
service:jmx:rmi:///jndi/rmi:(localhost|.*\.dev):1099/jmxrmi
----

In any case it is highly recommended to use a dedicated Jakarta EE servlet server for the JSR-160 proxy which
is secured by configuring the server authentication properly for this servlet. An unprotected Jolokia proxy
with too permissive allow list can be tricked to execute local code by a malicious attacker.
As said previously, the Jolokia proxy should be avoided if possible in favor of direct access over the Jolokia protocol.
