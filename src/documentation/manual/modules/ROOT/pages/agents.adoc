...
* `${ip}`, `${ip6}`, `${host}`, `${ip:<interface>}` (e.g., `${ip:eth0}`) to reference networking configuration. `${host}` is resolved to actual name only if `allowDnsReverseLookup` option is set to `true`. When not (which is the default), the value is the same as `${ip}`.
