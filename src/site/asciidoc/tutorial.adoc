...
installation and how to access the Agent using Jolokia Client.

=== Preparation

In order to start on a green field, we are installing a
fresh Tomcat in which the Jolokia WAR Agent is going to be
installed. Download Tomcat 11 from its
https://tomcat.apache.org/download-11.cgi[site,role=externalLink,window=_blank] and
extract it:

[source,bash]
----
$ unzip apache-tomcat-11.0.18.zip
$ cd apache-tomcat-11.0.18/
$ TC=$(pwd)
----

Download the Jolokia WAR-agent (without enabled security for the test purpose) and copy it into Tomcat's
`webapps/` directory:

[source,bash,subs="attributes,verbatim"]
----
$ artifact=jolokia-agent-war-unsecured
...
If `jolokia-agent-war-{jolokia-version}.war` was used, we'd need additional configuration. We need a user defined in
Tomcat's `UserDatabaseRealm` realm with `jolokia` role. Just add this line to `${TC}/conf/tomcat-users.xml`:
[source,xml]
----
...
[source,bash]
----
$ curl -u jolokia:jolokia -s http://localhost:8080/jolokia/read/java.lang:type=Runtime/VmVersion | jq .
{
  "request": {
...
public class JolokiaDemo {
    ...
    public static void main(String[] args) throws Exception {
        JolokiaClient client = new JolokiaClientBuilder().url("http://localhost:8080/jolokia")
                ...
                .password("jolokia")
                .build();
        JolokiaReadRequest request = new JolokiaReadRequest("java.lang:type=Memory", "HeapMemoryUsage");
        JolokiaReadResponse response = client.execute(request);
        Map<String, Long> vals = response.getValue();
        long used = vals.get("used");
        long max = vals.get("max");
        double usage = ((double) used * 100 / max);
        System.out.printf("Memory usage: used:%d/max:%d = %f%%%n", used, max, usage);
        client.close();
    }
}
----
...
* https://repo1.maven.org/maven2/org/jolokia/jolokia-core/{jolokia-version}/jolokia-core-{jolokia-version}.jar[jolokia-core-{jolokia-version},role=externalLink,window=_blank]
...
jolokia-core-{jolokia-version}.jar:\
jolokia-client-java-{jolokia-version}.jar

$ javac JolokiaDemo.java

$ java JolokiaDemo
Memory usage: used:26709664/max:16492003328 = 0.161955%
----

If you are using Maven, it is enough to include a single dependency:
