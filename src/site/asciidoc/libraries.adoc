...
We have now 7 groups of modules:

* https://github.com/jolokia/jolokia/tree/main/json[JSON parser,role=externalLink,window=_blank]
* https://github.com/jolokia/jolokia/tree/main/core[core module with common code,role=externalLink,window=_blank]
...
Single module that implements JSON parser/serializer. It replaces long-time used https://code.google.com/archive/p/json-simple/[json-simple,role=externalLink,window=_blank] library.
...
Single module that provides common classes to be used both by the server (agent) modules and client modules.
...
|`jolokia-core`
|Common serialization and utilities.
|===
...
|A set of built-in services which can be used to detect extra, non-discoverable (see `javax.management.MBeanServerFactory.findMBeanServer()`) `MBeanServer` instances, specific to given application server
...
|`jolokia-client-java`
|Dedicated client based on built-in JDK HTTP Client

|`jolokia-client-java-httpclient4`
...
|`jolokia-client-java-httpclient5`
|Dedicated client based on https://hc.apache.org/httpcomponents-client-5.5.x/index.html[Apache HTTP Client 5,role=externalLink,window=_blank]

|`jolokia-client-javascript-esm`
|Maven module containing modern Javascript libraries based on TypeScript, bundlers and ESM modules.

|`jolokia-client-javascript`
|Maven module containing legacy Javascript libraries
...
|Spring support library (Spring Boot 4, Spring Framework 7)

|`jolokia-support-spring3`
|Spring support library (Spring Boot 3, Spring Framework 6)
|===

=== Jolokia 1.x modules (no longer maintained)
