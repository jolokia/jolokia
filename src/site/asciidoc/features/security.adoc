...
allowed to access this agent. The name can be given as IPv4, IPv6 or
FQDN. A subnet mask can be given, too, for specifying a subnet
for the access. The netmask can be given either in CIDR format
(e.g "/16") or (for IPv4 only) has a full netmask (e.g. "/255.255.0.0").

When the `<remote>` list doesn't contain any `<host>` children, _no client is allowed access_.
If there's no `<remote>` element at all, _all clients are allowed access_.
...
* from `Forwarded`, `X-Forwarded-For` or `X-Real-IP` headers when Jolokia runs behind a reverse proxy
** the peer address of the reverse proxy is also verified, so its address should be included in the list
