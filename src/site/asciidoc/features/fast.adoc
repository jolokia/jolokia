...
NOTE: https://code.google.com/archive/p/json-simple/[json-simple,role=externalLink] is not a new library and since 2012 (last release) Java ecosystem was enriched with many JSON related libraries. We may review this subject, but for now, json-simple is really simple and sufficient (e.g., no complex data binding, thus no CVEs related to _gadgets_ and remote code execution).
