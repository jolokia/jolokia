...
`org.jolokia.server.core.detector.ServerDetector`:: is used to detect platform-specific `javax.management.MBeanServer` instances (which may not be detected using `javax.management.MBeanServerFactory.findMBeanServer(null)`)
