...
Chapter 5.3 of https://jcp.org/en/jsr/detail?id=3[JMX Specification 1.4,role=externalLink] defines two means of remote access to JVM's MBeanServer instances:
...
NOTE: JMX _connectors_ were originally defined in details in https://jcp.org/en/jsr/detail?id=160[JMX Remote API Specification (JSR-160),role=externalLink]. JSR-160 was then merged with https://jcp.org/en/jsr/detail?id=3[JMX Specification 1.4 (JSR-3),role=externalLink] itself.
...
OSGi agent:: An agent started from a bundle, which registers `org.jolokia.server.core.osgi.OsgiAgentServlet` using https://docs.osgi.org/specification/osgi.cmpn/8.1.0/service.servlet.html[OSGi CMPN Whiteboard Specification for Jakarta™ Servlet,role=externalLink].

JVM Agent uses JDK's https://docs.oracle.com/en/java/javase/11/docs/api/jdk.httpserver/com/sun/net/httpserver/package-summary.html[own HTTP server,role=externalLink]. WAR agent just runs inside Servlet container and OSGi agent uses whatever HTTP server is available in the OSGi runtime.
