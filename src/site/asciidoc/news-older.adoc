...
* The Jolokia link:reference/html/manual/proxy.html[JMX proxy mode,role=externalLink] is vulnerable
...
the link:reference/html/manual/proxy.html[Proxy Mode,role=externalLink] section of the reference manual.
...
wildcard support. You probably know link:reference/html/manual/protocol.html#read[pattern read
requests] which allow for fetching multiple patterns by
using patterns for MBean names and attributes (not to be
confused with link:reference/html/manual/protocol.html#post-request[bulk
...
pathes, but here is a try (another try can be found in the link:reference/html/manual/protocol.html#read[reference manual]):
...
The protocol for the discovery mechanism is also link:reference/html/manual/protocol.html#discovery[specified]
...
and is described link:reference/html/manual/mbeans.html#mbean-discovery[here].
...
* A new processing parameter link:reference/html/manual/protocol.html#processing-parameters[`ifModifiedSince`]
...
More details can be found here in the link:reference/html/manual/jmx.html#jvm-spring[reference manual].
...
The details can be found link:reference/html/manual/jmx.html[here].
