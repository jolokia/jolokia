...
That said, I still recommend the link:reference/html/manual/agents.html#agents-jvm[JAR agent,role=externalLink]
over the WAR agent for most cases, as it has much more flexible security options, included HTTPS encryption
with client certificate authentication.
See the link:reference/html/manual/agents.html#agents-jvm[reference manual,role=externalLink] for all
security options.
...
* The Jolokia link:reference/html/manual/proxy_mode.html[JMX proxy mode,role=externalLink] is vulnerable
to remote code execution in a specific scenario. Note that this only affects the
link:reference/html/manual/agents.html#agents-war[WAR agent,role=externalLink], which
has the proxy mode enabled by default.
The link:reference/html/manual/agents.html#agents-jvm[JVM agent,role=externalLink] is not affected as it
...
the link:reference/html/manual/proxy_mode.html[Proxy Mode,role=externalLink] section of the reference manual.
...
link:reference/html/manual/agents.html#agent-war-security[Security Setup,role=externalLink] chapter in the reference manual.
...
link:reference/html/manual/agents.html#agents-jvm[reference manual,role=externalLink].
...
More about this can be found in the link:reference/html/manual/agents.html#agents-jvm[reference manual,role=externalLink].
...
3.5. Thanks to href="https://github.com/FeiWongReed[Fei Wong Reed,role=externalLink]
...
session at https://cfp.devoxx.be/2014/talk/LVC-7834/Spicing_up_JMX_with_Jolokia[Devoxx,role=externalLink]
2014 in Antwerp.

Last announcement for now: I started a blog at https://ro14nd.de[,role=externalLink] about
...
https://github.com/rhuss/jolokia-extra[jolokia-extra,role=externalLink]
...
request,role=externalLink] from https://github.com/mplonka[Marcin Płonka,role=externalLink]
...
(hint: https://en.wikipedia.org/wiki/Cross-site_request_forgery[CSRF,role=externalLink]).
...
supporting this discovery mode is https://search.cpan.org/~roland/jmx4perl/lib/JMX/Jmx4Perl.pm[Jmx4Perl,role=externalLink]
...
I would
ask you to open an https://github.com/jolokia/jolokia/issues[issue,role=externalLink] in case of any problems.
...
https://github.com/jolokia/jolokia/commit/98e62c0478b166d188e0c9f173b278a59eed1fc8#diff-cdf118c56620542b85bc48650eb8ddb6R162[single char,role=externalLink]
...
https://twitter.com/ro14nd[me,role=externalLink]
in Antwerp. Ping me or look for the guy with the
https://jolokia.spreadshirt.de/men-s-hooded-jacket-A17809556/[Jolokia hoodie,role=externalLink] ;-)
...
BTW, the slides to my talk for the small but very fine https://www.jayday.de[JayDay 2013,role=externalLink] are
https://www.jolokia.org/jayday-2013[online,role=externalLink], too.
...
bottom of a https://www.jolokia.org/jayday-2013/#/step-20[demo slide,role=externalLink].
...
https://github.com/jolokia/jolokia/tree/2.0[branch,role=externalLink]
...
In the meantime, you can also see Jolokia live at https://www.jayday.de/[JayDay,role=externalLink] where I will give
a talk about
...
https://github.com/jolokia/jolokia/tree/notification[notification,role=externalLink]). There
...
entered
https://jolokia.idea.informer.com/[jolokia.idea.informer.com,role=externalLink] has been transformed into
GitHub issues.
...
found on our
https://www.consol.de/open-source-monitoring/schulungen/java-monitoring-mit-nagios/[web site,role=externalLink]
(in german).

And finally a very **hot** recommendation: Please have a look at https://hawt.io[hawt.io,role=externalLink]
a super cool
...
Jolokia
* I'm going to talk about Jolokia at https://www.jayday.de/?lang=en[jayday 2012,role=externalLink], a brand new,
low cost
...
https://jolokia.spreadshirt.de/[Jolokia Shop,role=externalLink]
...
https://github.com/square/cubism/[Cubism,role=externalLink], a fine
time series charting library based on
https://d3js.org/[d3.js,role=externalLink]. Cubism provides
support for an innovative charting type, the
https://vis.berkeley.edu/papers/horizon/[horizon charts,role=externalLink]:
...
Jolokia uses also a
https://github.com/jolokia/jolokia/actions[GitHub workflow,role=externalLink]
build in addition to our own
https://labs.consol.de/jenkins[CI Server,role=externalLink]. (Did I
mentioned already, that we have a quite I high
https://sonarcloud.io/project/overview?id=org.jolokia%3Ajolokia[Sonar,role=externalLink]
score ?).

That's it for now. The next months of my open-source work
will be spent now on
https://github.com/rhuss/aji[Ají,role=externalLink], Jolokia's new
