...
* https://www.youtube.com/watch?v=PDf2mqxOeMk[Jolokia from JMC,role=externalLink,window=_blank]
* https://www.youtube.com/watch?v=ALkMdEPPg1U[Connect Visual VM to a JVM running in Kubernetes using Jolokia,role=externalLink,window=_blank]
* https://www.youtube.com/watch?v=IkxDErc23lw[Connect Java Mission Control to a JVM in Kubernetes,role=externalLink,window=_blank]

All kudos go to https://github.com/skarsaune[Martin Skarsaune,role=externalLink,window=_blank] for this great new feature!

Thanks to the great work of https://github.com/grgrzybek[Grzegorz Grzybek,role=externalLink,window=_blank] Jolokia works now also
for Java 11, and supports now the full range from Java 6 up to Java 11. All harmful reflection code has been
replaced.

There are many other little gems added by great people, please checkout the https://github.com/jolokia/jolokia/releases[release notes,role=externalLink,window=_blank] and/or the link:changes-report.html[changelog].
...
Said that it's currently a perfect moment to jump in to boost Jolokia development. We've got the https://github.com/jolokia/[jolokia,role=externalLink,window=_blank] donated, so one of the next larger moves would move this personal pet project to a more solid foundation.
...
As https://matmannion.com/jolokiapwn/"[reported,role=externalLink,window=_blank] by security consultant Mat Mannion, there is a massive set of Jolokia agents reachable unsecured from the Internet.
While its obviously not recommended to run Jolokia unsecured or even expose it publicly, authentication is enabled now by default in `jolokia.war`. So in order to continue to the xref:agent/war.adoc[WAR-agent] you have to setup
...
https://repo1.maven.org/maven2/org/jolokia/jolokia-war-unsecured/1.6.0/[jolokia-unsecured.war,role=externalLink,window=_blank].
Of course you are still free to mangle the `web.xml` within either war agent.

That said, I still recommend the link:reference/html/manual/agents.html#agents-jvm[JAR agent,role=externalLink,window=_blank]
over the WAR agent for most cases, as it has much more flexible security options, included HTTPS encryption
with client certificate authentication.
See the link:reference/html/manual/agents.html#agents-jvm[reference manual,role=externalLink,window=_blank] for all
security options.

[#Security_fixes_with_1_5_0]
...
https://www.gdssecurity.com/[Gotham Digital Science,role=externalLink,window=_blank]:

* The Jolokia link:reference/html/manual/proxy_mode.html[JMX proxy mode,role=externalLink,window=_blank] is vulnerable
to remote code execution in a specific scenario. Note that this only affects the
link:reference/html/manual/agents.html#agents-war[WAR agent,role=externalLink,window=_blank], which
has the proxy mode enabled by default.
The link:reference/html/manual/agents.html#agents-jvm[JVM agent,role=externalLink,window=_blank] is not affected as it
...
the link:reference/html/manual/proxy_mode.html[Proxy Mode,role=externalLink,window=_blank] section of the reference manual.
...
link:reference/html/manual/agents.html#agent-war-security[Security Setup,role=externalLink,window=_blank] chapter in the reference manual.
...
issue at the https://github.com/jolokia/jolokia[GitHub project,role=externalLink,window=_blank].
...
more
relevant as https://ro14nd.de/java-management-is-dead[Java EE Management,role=externalLink,window=_blank] (JSR-373) will never
come.
Of course, as times goes by, alternative monitoring interfaces for Java (like to
https://prometheus.io/[Prometheus,role=externalLink,window=_blank])
...
* The https://github.com/jolokia/jolokia/issues/258[annoying issue,role=externalLink,window=_blank] with the Jolokia agent when
running
...
based system, https://github.com/jolokia/jolokia/issues/new[let us know,role=externalLink,window=_blank].
...
features at https://2016.javazone.no/program/jolokia-2-0[JavaZone,role=externalLink,window=_blank] in September.
...
https://search.maven.org/remotecontent?filepath=org/jolokia/jolokia-agent-jvm/2.0.0-M1/jolokia-agent-jvm-2.0.0-M1-agent.jar[JAR,role=externalLink,window=_blank]
and
https://search.maven.org/remotecontent?filepath=org/jolokia/jolokia-agent-war/2.0.0-M1/jolokia-agent-war-2.0.0-M1.war[WAR,role=externalLink,window=_blank]
agent which are supposed to be drop in replacements for Jolokia 1.3.2.

More information can be found on my https://ro14nd.de[Blog,role=externalLink,window=_blank]. Soon there will be also
...
The changes affect the xref:agent/jvm.adoc[JVM agent] only
and are explained in the
link:reference/html/manual/agents.html#agents-jvm[reference manual,role=externalLink,window=_blank].
...
More about this can be found in the link:reference/html/manual/agents.html#agents-jvm[reference manual,role=externalLink,window=_blank].
Note, that the parameter `authenticationClass` has been renamed to
`authClass`
for consistencies sake. Please raise an https://github.com/jolokia/jolokia/issues/new[issue,role=externalLink,window=_blank] if
...
release. This is mostly a link:changes-report.html[bug fix release] with some new features:
...
large enough,role=externalLink,window=_blank]. So the new default behaviour is to have **no limit** at
...
former employer https://www.consol.com[ConSol,role=externalLink,window=_blank]. Without the support donated by ConSol Jolokia
would
...
https://www.slideshare.net/roland.huss/jolokia-devoxx2014/11[Jolokia,role=externalLink,window=_blank]
https://github.com/jolokia/jolokia/wiki[2.0,role=externalLink,window=_blank]
...
setup. Kudos to https://github.com/nevenr[Neven Radovanović,role=externalLink,window=_blank] for
providing a patch.
* The Mule agent has been updated to support Mule
3.5. Thanks to href="https://github.com/FeiWongReed[Fei Wong Reed,role=externalLink,window=_blank]
...
session at https://cfp.devoxx.be/2014/talk/LVC-7834/Spicing_up_JMX_with_Jolokia[Devoxx,role=externalLink,window=_blank]
2014 in Antwerp.

Last announcement for now: I started a blog at https://ro14nd.de[,role=externalLink,window=_blank] about
...
wildcard support. You probably know link:reference/html/manual/jolokia_protocol.html#read[pattern read
requests] which allow for fetching multiple patterns by
using patterns for MBean names and attributes (not to be
confused with link:reference/html/manual/jolokia_protocol.html#post-request[bulk
...
pathes, but here is a try (another try can be found in the link:reference/html/manual/jolokia_protocol.html#read[reference manual]):
...
https://github.com/rhuss/jolokia-extra[jolokia-extra,role=externalLink,window=_blank]
...
request,role=externalLink,window=_blank] from https://github.com/mplonka[Marcin Płonka,role=externalLink,window=_blank]
...
Management,role=externalLink,window=_blank] was a cool attempt to standardize naming and
...
with twitter (https://twitter.com/jolokia_jmx[@jolokia_jmx,role=externalLink,window=_blank]) or
...
This minor release fixes some bugs and brings some link:changes-report.html[smaller features]:
...
recommended to use a link:reference/html/manual/security.html#security-policy[jolokia-access.xml] policy in
production
...
(hint: https://en.wikipedia.org/wiki/Cross-site_request_forgery[CSRF,role=externalLink,window=_blank]).
...
the excellent https://hawt.io[hawt.io,role=externalLink,window=_blank] will probably use it one way or the other. In fact, it was
hawt.io
which put me https://github.com/jolokia/jolokia/pull/126[on track,role=externalLink,window=_blank] for this nice little feature ;-)

Discovery is enabled by default for the link:reference/html/manual/agents.html#agents-jvm[JVM agent], but not
for
the link:reference/html/manual/agents.html#agents-war[WAR agent].
It can be easily enabled for the WAR agent by using servlet init parameters, system properties or environment
variables.
All the nifty details can be found in the link:reference/html/manual/index.html[reference manual].

The protocol for the discovery mechanism is also link:reference/html/manual/jolokia_protocol.html#discovery[specified]
in the reference manual. One of the first clients
supporting this discovery mode is https://search.cpan.org/~roland/jmx4perl/lib/JMX/Jmx4Perl.pm[Jmx4Perl,role=externalLink,window=_blank]
...
and is described link:reference/html/manual/jolokia_mbeans.html#mbean-discovery[here].

This feature has been tested in various environments, but since low level networking can be, well, "painful",
I would
ask you to open an https://github.com/jolokia/jolokia/issues[issue,role=externalLink,window=_blank] in case of any problems.
...
https://github.com/jolokia/jolokia/commit/98e62c0478b166d188e0c9f173b278a59eed1fc8#diff-cdf118c56620542b85bc48650eb8ddb6R162[single char,role=externalLink,window=_blank]
...
https://twitter.com/ro14nd[me,role=externalLink,window=_blank]
in Antwerp. Ping me or look for the guy with the
https://jolokia.spreadshirt.de/men-s-hooded-jacket-A17809556/[Jolokia hoodie,role=externalLink,window=_blank] ;-)
...
BTW, the slides to my talk for the small but very fine https://www.jayday.de[JayDay 2013,role=externalLink,window=_blank] are
https://www.jolokia.org/jayday-2013[online,role=externalLink,window=_blank], too.
...
bottom of a https://www.jolokia.org/jayday-2013/#/step-20[demo slide,role=externalLink,window=_blank].
...
https://github.com/jolokia/jolokia/tree/2.0[branch,role=externalLink,window=_blank]
...
In the meantime, you can also see Jolokia live at https://www.jayday.de/[JayDay,role=externalLink,window=_blank] where I will give
a talk about
...
* A new processing parameter link:reference/html/manual/jolokia_protocol.html#processing-parameters[`ifModifiedSince`]
...
`serializeException`
is active.
* The JavaScript client's link:reference/html/manual/clients.html#js-poller[`jolokia.register()`]
...
https://github.com/jolokia/jolokia/tree/notification[notification,role=externalLink,window=_blank]). There
...
More details can be found here in the link:reference/html/manual/jolokia_jmx.html#jvm-spring[reference manual].
...
image::images/jconsole-json-mbean.png[role="text-center"]

The details can be found link:reference/html/manual/jolokia_jmx.html[here].
...
https://github.com/jolokia/jolokia/issues[GitHub,role=externalLink,window=_blank].
Since I'm currently collecting features for 2.0, it's a good time for feature requests ;-). All ideas
entered
https://jolokia.idea.informer.com/[jolokia.idea.informer.com,role=externalLink,window=_blank] has been transformed into
GitHub issues.
...
found on our
https://www.consol.de/open-source-monitoring/schulungen/java-monitoring-mit-nagios/[web site,role=externalLink,window=_blank]
(in german).

And finally a very **hot** recommendation: Please have a look at https://hawt.io[hawt.io,role=externalLink,window=_blank]
a super cool
...
* The new xref:talks.adoc[Talks and Screencast] section collects some fancy multimedia introducing
Jolokia
* I'm going to talk about Jolokia at https://www.jayday.de/?lang=en[jayday 2012,role=externalLink,window=_blank], a brand new,
low cost
...
https://jolokia.spreadshirt.de/[Jolokia Shop,role=externalLink,window=_blank]
...
https://github.com/square/cubism/[Cubism,role=externalLink,window=_blank], a fine
time series charting library based on
https://d3js.org/[d3.js,role=externalLink,window=_blank]. Cubism provides
support for an innovative charting type, the
https://vis.berkeley.edu/papers/horizon/[horizon charts,role=externalLink,window=_blank]:

image::images/jolokia_cubism_demo_2.png[role="text-center"]

image::images/jolokia_cubism_demo_1.png[role="text-center"]

A very cool live demo where a Jolokia JavaScript client
fetches live data from our servers and plot it with Cubism
can be found on this
xref:client/javascript-cubism.adoc[demo page].
The documentation can be found in the link:reference/html/manual/clients.html#js-cubism[reference manual].

Jolokia uses also a
https://github.com/jolokia/jolokia/actions[GitHub workflow,role=externalLink,window=_blank]
build in addition to our own
https://labs.consol.de/jenkins[CI Server,role=externalLink,window=_blank]. (Did I
mentioned already, that we have a quite I high
https://sonarcloud.io/project/overview?id=org.jolokia%3Ajolokia[Sonar,role=externalLink,window=_blank]
score ?).

That's it for now. The next months of my open-source work
will be spent now on
https://github.com/rhuss/aji[Ají,role=externalLink,window=_blank], Jolokia's new
