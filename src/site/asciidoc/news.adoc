...
=== Jolokia 2.6.3 released

[.news-date]
2026-09-21

A new version of Jolokia has been released with fixed security declaration for `/config` endpoint. Apache Tomcat verifies
if all the combinations of URI mappings and methods are _covered_ and now Jolokia provides a full coverage.

Refer to https://jolokia.org/changes-report.html#a2.6.3[changelog] and https://github.com/jolokia/jolokia/releases/tag/v2.6.3[GitHub release notes] for more details.

=== Jolokia 2.6.2 and 1.7.3 released with security fixes

[.news-date]
2026-09-02

New version of Jolokia comes with a fix for https://nvd.nist.gov/vuln/detail/cve-2026-84218[CVE-2026-84218]. This
CVE is a complete fix for previous https://nvd.nist.gov/vuln/detail/cve-2018-1000130[CVE-2018-1000130] related to
Jolokia JSR-160 proxy. Previous regular-expression based deny-list was not covering all cases. +
The report was created by https://github.com/ByteHackr[Sandipan Roy (Red Hat)] and Mirko Swillus (https://alpha-omega.dev[]).

Now _all_ target JMX Service URLs are denied by default and the only way to allow _any_ target URL is to use
`jsr160ProxyAllowedTargets` option. It allows to point to a file (or classpath resource) with a list of allowed
JMX Service URL patterns.

While Jolokia 1.x should be fully replaced by Jolokia 2, we've also released a security update for this old branch in version 1.7.3.

Additionally (only in 2.6.2) the `jolokia-bom` is now _flattened_ (thanks https://github.com/mrozanc[Marc Rozanc]!).

Refer to https://jolokia.org/changes-report.html#a2.6.2[changelog] and https://github.com/jolokia/jolokia/releases/tag/v2.6.2[GitHub release notes] for more details.

=== Jolokia 2.6.1 released

[.news-date]
2026-08-11

We want to announce a new release of Jolokia that adds and fixes some really old issues.

https://github.com/skarsaune[Martin Skarsaune] improved Jolokia Kubernetes JMX adapter which no longer depends on https://hc.apache.org/httpcomponents-client-4.5.x/index.html[Apache Http Client 4].

https://github.com/GChuf[Gašper Čefarin] fixed some problems and optimized Jolokia's own JSON parser.

In the area of Security:

* Jolokia adds support for https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Fetch_metadata[Fetch Metadata]
* CORS protocol handling in Jolokia properly responds to pre-flight `OPTIONS` request
* Jolokia.js client library fixes handling of credentials in cross-origin requests
* Jolokia supports reverse proxy headers (like `X-Forwarded-For`) and there's a new dedicated section in https://jolokia.org/reference/html/manual/security.html#_ip_based_restrictions[the security chapter of Jolokia Reference Manual].

And finally, there's a surprising addition to Jolokia JVM Agent based on JDK HTTP Server - we now provide proper ... `favicon.ico` resource. The reason is related to how this JDK HTTP Server handles non-http traffic and how Firefox tries to fetch the icon over https first... See more details under https://github.com/jolokia/jolokia/issues/1029[jolokia/jolokia#1029].

Refer to https://jolokia.org/changes-report.html#a2.6.1[changelog] and https://github.com/jolokia/jolokia/releases/tag/v2.6.1[GitHub release notes] for more details.

=== Jolokia 2.6.0 released

[.news-date]
2026-04-29

New minor (but not micro) release of Jolokia is released. The important change is the removal of `org.jolokia:jolokia-client-javascript`
module which included Jolokia.js library in original form. Since Jolokia 2.1.0 we include the _modern_ JavaScript library
written in TypeScript and published https://www.npmjs.com/package/jolokia.js[to npmjs.com]. Starting with Jolokia 2.6.0 we
no longer have to include the old versions (security scanners were less and less happy with the related `package.json`...).

Additional important change is related to https://openjdk.org/jeps/411[JEP-411: Deprecate the Security Manager for Removal].
While Jolokia did not use SecurityManager, JEP-411 also includes breaking change to `javax.security.auth.Subject` API and we now
ensure that Jolokia works with latest JDK (where `javax.security.auth.Subject.getSubject()` throws `UnsupportedOperationException`).

Refer to https://jolokia.org/changes-report.html#a2.6.0[changelog] and https://github.com/jolokia/jolokia/releases/tag/v2.6.0[GitHub release notes] for more details.

=== Jolokia 2.5.1 released

[.news-date]
2026-02-18

Just a tiny fix for `OpenType` information used in `jolokia-client-jmx-connector`. Now `JConsole` should be able to
view CompositeData even if some items use unknown types.

Refer to https://jolokia.org/changes-report.html#a2.5.1[changelog] and https://github.com/jolokia/jolokia/releases/tag/v2.5.1[GitHub release notes] for more details.
...
* The Jolokia link:link:https://jolokia.org/reference/html/manual/proxy_mode.html[JMX proxy mode,role=externalLink,window=_blank] is vulnerable
to remote code execution in a specific scenario. Note that this only affects the
link:link:https://jolokia.org/reference/html/manual/agents.html#agents-war[WAR agent,role=externalLink,window=_blank], which
has the proxy mode enabled by default.
The link:link:https://jolokia.org/reference/html/manual/agents.html#agents-jvm[JVM agent,role=externalLink,window=_blank] is not affected as it
...
the link:link:https://jolokia.org/reference/html/manual/proxy_mode.html[Proxy Mode,role=externalLink,window=_blank] section of the reference manual.
...
link:link:https://jolokia.org/reference/html/manual/agents.html#agent-war-security[Security Setup,role=externalLink,window=_blank] chapter in the reference manual.
