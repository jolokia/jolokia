...
* With the new `includeRequest` processing/request parameter, we can tell Jolokia not to return incoming requests under `the "request"` field of the response (defaults to `false`)
...
* Added ability to declare (in `jolokia-access.xml`) that some MBans simply should not be returned in `search` and `list` operations.
