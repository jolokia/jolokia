...
=== Jolokia 2.2.0 released

[.news-date]
2025-01-23

We're happy to release Jolokia 2.2.0! This is new minor release, as the scope of changes is a bit too wide for just 2.1.3 release.
...
Spring Boot depencency of `jolokia-support-spring` module has been changed to 3.4.1, but that's only declared dependency. Jolokia works well with all Spring Boot 3 versions.
...
Also, similarly to https://docs.spring.io/spring-boot/reference/features/external-config.html[Spring Boot configuration], Jolokia looks up configuration options in differente sources/locations (in order of increasing priority - later sources override previous ones):
...
With this release, current version of `jolokia.js` npm package is 2.2.0 - even if it doesn't bring any changes.

Refer to https://jolokia.org/changes-report.html#a2.2.0[changelog] for detailed information about the release.
