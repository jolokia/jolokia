 ...
 * @since Apr 19, 2009
 */
public class JSONObjectExtractor implements Extractor {
    ...
    public Object extractObject(ObjectToJsonConverter pConverter, Object pValue,
        ...
        JSONObject map = (JSONObject) pValue;
        int length = pConverter.getCollectionLength(map.length());
    ...
    private JSONObject extractMapValues(ObjectToJsonConverter pConverter, Deque<String> pPathParts, boolean jsonify, JSONObject pMap, int pLength) throws AttributeNotFoundException {
        ...
        for(String key : pMap.keySet()) {
            ...
            try {
                ret.put(key,
                        pConverter.extractObject(pMap.get(key), paths, jsonify));
                if (++i > pLength) {
                    break;
                }
    ...
    private Object extractMapValueWithPath(ObjectToJsonConverter pConverter, Object pValue, Deque<String> pPathParts, boolean jsonify, JSONObject pMap, String pPathParth) throws AttributeNotFoundException {
        for (String key : pMap.keySet()) {
            // We dont access the map via a lookup since the key
            // are potentially object but we have to deal with string
            // representations
            if(pPathParth.equals(key)) {
                return pConverter.extractObject(pMap.get(key), pPathParts, jsonify);
            }
        }
    ...
    public Object setObjectValue(StringToObjectConverter pConverter, Object pMap, String pKey, Object pValue)
        ...
        JSONObject map = (JSONObject) pMap;
