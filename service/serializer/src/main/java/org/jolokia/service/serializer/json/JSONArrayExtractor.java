 ...
 * @since Apr 19, 2009
 */
public class JSONArrayExtractor implements Extractor {
    ...
    public Object extractObject(ObjectToJsonConverter pConverter, Object pValue, Deque<String> pPathParts, boolean jsonify)
            throws AttributeNotFoundException {
        JSONArray list = (JSONArray) pValue;
        int length = pConverter.getCollectionLength(list.length());
    ...
    private Object extractWithPath(ObjectToJsonConverter pConverter, JSONArray pList, Deque<String> pStack, boolean jsonify, String pPathPart) throws AttributeNotFoundException {
        try {
            ...
            ValueFaultHandler faultHandler = pConverter.getValueFaultHandler();
            return faultHandler.handleException(
                    new AttributeNotFoundException("Index '" + pPathPart +  "' is out-of-bound for a list of size " + pList.length()));
        }
    }

    private Object extractListAsJson(ObjectToJsonConverter pConverter, JSONArray pList, Deque<String> pPath, int pLength) throws AttributeNotFoundException {
